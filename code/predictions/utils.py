...
from model import Omnipath_Node2Vec

# from ..preprocessing.utils import get_pathway_encodings
...
def get_Omnipath_embeddings(nodes, interactions, reproduce=None, save=None, lr=0.01):
    ...
    device = "cpu"
    torch.manual_seed(0)
    Omnipath_data, Omnipath_nodes, Omnipath_interactions = make_dataset(nodes, interactions, first=False,
                                                                        pathway_encode=False)
...
def get_cell_LR_embeddings(matrix, meta, nodes, interactions, total_embeddings_df, Omnipath_nodes,
                           Omnipath_interactions, spatial=None, reproduce=None, save=None):
    device = 'cpu'
    torch.manual_seed(0)
    truth_info = pd.DataFrame(np.zeros((Omnipath_nodes.shape[0], Omnipath_nodes.shape[0])),
                              index=Omnipath_nodes["identifier"].tolist(),
